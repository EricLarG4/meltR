...
ui <- dashboardPage(
    ...
    dashboardSidebar(
        ...
        conditionalPanel(
            ...
            box(
                ...
                tags$a(
                    href = 'https://github.com/EricLarG4/meltR/raw/master/demo_input.xlsx',
                    '1. Download the input file and replace the demo data with yours'
                ),
                p("2. Make sure to fill in the header cells"),
                p(
                    '3. Upload the data using the raw data box in the left sidebar'
                ),
                p(
                    '4. Select the data to process with the input data hover panel'
                ),
                h4('II. Data processing'),
                p('1. Derivate the data with the blue button'),
                p('2. Initialize the fitting with the orange button'),
                p(
                    '3. If necessary, correc the initialization. In particular,
          make sure the initial Tm is close to its expected value'
                ),
                p('4. Start the fitting with the red button'),
                h4('III. Results'),
                p(
                    '1. Review the fitting. In particular, check that the baselines are consistent with the data'
                ),
                p('2. ...')
            ),
            box(
            ...
            box(
                ...
                switchInput(
                    inputId = "toggle.baseline",
                    label = "toggle baselines",
                    value = TRUE
                ),
                sliderInput(
                    "temp.ff",
                    "Temperature (°C) for folded fraction",
                    min = 0,
                    max = 100,
                    value = 37
                ),
    ...
    dashboardBody(
        ...
        tags$style(
            '.irs-bar{background: linear-gradient(to right, steelblue, tomato)}'
        ),
...
server <- function(input, output, session) {
    ...
    melt.input <- reactive({
        wide.input <- read_excel(input.file()$datapath, sheet = "UV-melting")
        ...
        for (i in 1:(ncol(raw.data) / 3)) {
            ...
            buffer <- raw.data %>%
                ...
                mutate(
                    descriptors[[1, n + 1]], #adds columns for descriptors
                    descriptors[[2, n + 1]],
                    descriptors[[3, n + 1]],
                    descriptors[[4, n + 1]],
                    descriptors[[5, n + 1]],
                ...
                convert(num(
                    ...
                    'melt.l',
                    'melt.c'
                )) %>% #converts some columns to numeric type
                drop_na('T.unk')

            #binds data
            data.collector <- rbind(data.collector, buffer, make.row.names = F)
        }
    ...
    melt <- reactive({
        melt <- melt.input() %>%
            ...
            mutate(
                ramp = if_else(lead(T.unk) > T.unk, 'heating', 'cooling')
            ) %>% #ramp determination
            ...
            add_column(blk.sub = blk.subtract(), toggle.eps = toggle.eps()) %>%
            group_by(id) %>%
        ...
        if (input$raw.smooth == T) {
            melt %>%
                group_by(id) %>%
                mutate(
                    abs.melt = predict(
                        sm.spline(T.K, abs.melt),
                        T.K,
                        0
                    )
                ) %>%
    ...
    melt.filtered <- reactive({
        if (file.toggle() == 'no') {
            ...
            melt.filtered.buffer <- melt() %>% #input data filtering
                ...
                filter(
                    T.K > min(input$slider.therm) &
                        T.K < max(input$slider.therm)
                )
    ...
    output$p.melt.filtered <- renderPlot({
        if (file.toggle() == 'no') {
            ...
            p45 <- ggplot(
                ...
                geom_point(
                    size = input$size.dot.melt,
                    alpha = input$alpha.dot.melt
                ) +
                scale_color_d3() +
                theme_pander() +
                xlab("Temperature (°C)")

            if (input$toggle.eps == TRUE) {
                #modifies axes titles
        ...
        if (input$drv.type == T) {
                ...
                mutate(
                    emp = abs(
                        predict(
                            ...
                            T.K,
                            1
                        ) #get deriv against temperature from spline function
                    )
                )
            ...
            for (i in unique(melt.filtered()$id)) {
                buffer <- sav(
                    ...
                    poly.order = input$sav.order, #poly 2nd order
    ...
    output$p.melt.derivative <- renderPlot({
        p46 <- ggplot(
            melt.derivative(),
            aes(T.K - 273.15, emp, color = id, shape = ramp)
        ) +
            geom_point(
                size = input$size.dot.melt,
                alpha = input$alpha.dot.melt
        ...
        if (input$toggle.eps == TRUE) {
            ...
            p46 <- p46 +
                ylab(bquote(
                    Delta * epsilon * '/' * Delta * 'T (' * M^-1 ~
                        cm^-1 * K^-1 * ')'
                ))
        } else {
            p46 <- p46 +
                ylab(bquote(Delta * A * '/' * Delta * 'T (' * K^-1 * ')'))
        }
    ...
    nlfit.melt <- eventReactive(input$bttn.fit.melt, {
        ...
        for (i in unique(melt.filtered()$id)) {
            ...
            P3s <- as.vector(fit.melt.init.par$P3.init) /
                (melt.c / 1E6 * melt.l) #convert initial parameters to molar abs coeff.
            ...
            ms <- nls(
                ...
                fit.melt.input.buffer$abs.melt ~
                    (P3 + P4 * fit.melt.input.buffer$T.K) *
                        1 /
                        (1 +
                            exp(
                                -P1 *
                                    (1 - fit.melt.input.buffer$T.K / P2) /
                                    (8.31451 * fit.melt.input.buffer$T.K)
                            )) +
                        (P5 + P6 * fit.melt.input.buffer$T.K) *
                            exp(
                                -P1 *
                                    (1 - fit.melt.input.buffer$T.K / P2) /
                                    (8.31451 * fit.melt.input.buffer$T.K)
                ...
                nls.control(maxiter = input$nb.it.melt.fit, warnOnly = T)
            )

            #buffers the fit results
            fit.melt.output.buffer <- data.frame(
                id = i,
                nb.data.pt = nobs(ms),
                init.Tm = P2s,
    ...
    output$nlfit.melt.results <- DT::renderDT(server = FALSE, {
        datatable(
            ...
            options = list(
                ...
                columnDefs = list(list(
                    visible = FALSE,
                    targets = c(3, 6, 8, 10, 12, 14, 16, 17, 18, 19, 20)
                ))
            )
        ) %>%
    ...
    fit.melt.result.df <- reactive({
        ...
        # else {
        left_join(melt.filtered(), nlfit.melt(), by = c("id")) %>% #join fit result with raw data (only selected ids)
            mutate(
                folded.fraction = (1 /
                    (1 + exp(-P1 * (1 - T.K / P2) / (8.31451 * T.K))))
            ) %>% #folded fraction
            mutate(
                folded.fraction.base = (P5 + P6 * T.K - abs.melt) /
                    (P5 + P6 * T.K - P3 - P4 * T.K)
            ) %>% #baseline corrected folded fraction
            #fitted line
            mutate(
                raw.fit.y = (P3 + P4 * T.K) *
                    1 /
                    (1 + exp(-P1 * (1 - T.K / P2) / (8.31451 * T.K))) +
                    (P5 + P6 * T.K) *
                        exp(-P1 * (1 - T.K / P2) / (8.31451 * T.K)) /
                        (1 + exp(-P1 * (1 - T.K / P2) / (8.31451 * T.K)))
            ) %>%
            mutate(low.T.baseline = P3 + P4 * T.K) %>%
            mutate(high.T.baseline = P5 + P6 * T.K) %>%
            filter(
                T.K > min(input$slider.therm) & T.K < max(input$slider.therm)
            )
        # }
    })
    ...
    output$processed.data <- renderDT(server = FALSE, {
        fit.melt.result.df() %>%
            ...
            formatRound(
                c('model', 'folded fraction', 'temperature'),
                digits = 3
            )
    })
    ...
    p.raw.melt.fit <- reactive({
        ...
        p0 <- ggplot(fit.melt.result.df()) +
            geom_point(
                aes(T.K - 273.15, abs.melt, color = id),
                size = input$size.dot.melt,
                alpha = input$alpha.dot.melt,
                shape = 16
            ) + #plots the experimental data
            geom_line(
                aes(x = T.K - 273.15, y = raw.fit.y, color = id),
                size = input$size.line.melt,
                alpha = input$alpha.line.melt
            ) +
            xlab("Temperature (°C)") +
        ...
        if (input$toggle.baseline == T) {
            p0 <- p0 +
                geom_line(
                    aes(x = T.K - 273.15, y = low.T.baseline, color = id),
                    size = input$size.baseline.melt,
                    alpha = input$alpha.baseline.melt,
                    linetype = "dashed"
                ) +
                geom_line(
                    aes(x = T.K - 273.15, y = high.T.baseline, color = id),
                    size = input$size.baseline.melt,
                    alpha = input$alpha.baseline.melt,
                    linetype = "dashed"
                )
    ...
    p.folded.modeled <- reactive({
        if (file.toggle() == 'no') {
            ...
            p44 <- ggplot(fit.melt.result.df()) +
                geom_point(
                    aes(T.K - 273.15, folded.fraction, color = id),
                    size = input$size.dot.melt - 2,
                    alpha = input$alpha.dot.melt,
                    shape = 16
                ) + #plots the experimental data
    ...
    p.folded.melt.fit <- reactive({
        if (file.toggle() == 'no') {
            ...
            p43 <- ggplot(fit.melt.result.df()) +
                geom_point(
                    aes(T.K - 273.15, folded.fraction.base, color = id),
                    size = input$size.dot.melt,
                    alpha = input$alpha.dot.melt,
                    shape = 16
                ) + #plots the experimental data
    ...
    theta.intersect <- reactive({
        fit.melt.result.df() %>%
            ...
            summarise(
                tm.inter = approxfun(
                    x = folded.fraction.base[T.K > 290],
                    y = T.K[T.K > 290]
                )(0.5),
                tm.inter.c = tm.inter - 273.15,
                theta.temp = approxfun(x = T.K, y = folded.fraction.base)(
                    input$temp.ff + 273.15
                )
            )
    })
    ...
    output$theta.deriv <- renderPlot({
        p77 <- ggplot(
            theta.deriv(),
            aes(recip.T, rM, color = id, shape = ramp)
        ) +
            geom_point(
                size = input$size.dot.melt,
                alpha = input$alpha.dot.melt
    ...
    theta.deriv.max <- reactive({
        theta.deriv() %>%
            group_by(id) %>%
            filter(rM == max(rM)) %>%
            summarise(tm.deriv = 1 / recip.T, tm.deriv.c = tm.deriv - 273.15)
    })
    ...
    output$p.melt.filtered.base <- renderPlot({
        if (file.toggle() == 'no') {
            ...
            p666 <- ggplot(
                ...
                geom_vline(
                    aes(xintercept = input$start.low[1]),
                    color = 'springgreen4',
                    size = 1
                ) +
                geom_vline(
                    aes(xintercept = input$start.low[2]),
                    linetype = 'dashed',
                    color = 'springgreen4',
                    size = 1
                ) +
                geom_vline(
                    aes(xintercept = input$end.low[1]),
                    linetype = 'dashed',
                    color = 'purple4',
                    size = 1
                ) +
                geom_vline(
                    aes(xintercept = input$end.low[2]),
                    color = 'purple4',
                    size = 1
                ) +
                geom_vline(
                    aes(xintercept = input$start.high[1]),
                    color = 'springgreen4',
                    size = 1
                ) +
                geom_vline(
                    aes(xintercept = input$start.high[2]),
                    linetype = 'dashed',
                    color = 'springgreen4',
                    size = 1
                ) +
                geom_vline(
                    aes(xintercept = input$end.high[1]),
                    linetype = 'dashed',
                    color = 'purple4',
                    size = 1
                ) +
                geom_vline(
                    aes(xintercept = input$end.high[2]),
                    color = 'purple4',
                    size = 1
                ) +
                scale_color_d3() +
    ...
    baseline.set <- reactive({
        basegenR(
            start.low = input$start.low + 273.15,
    ...
    p.bases <- reactive({
        p345 <- baseline.gen() %>%
            ...
            geom_point(aes(y = abs), color = 'grey', show.legend = FALSE) +
            geom_line(
                aes(y = med.bl, color = id.bl, group = factor(id.bl)),
                linetype = 'dashed',
                size = 1,
                alpha = 0.75,
                show.legend = TRUE
            ...
            geom_line(
                aes(y = low.bl, color = id.bl, group = (id.bl)),
                size = 1,
                alpha = 0.75,
                show.legend = FALSE
            ) +
            geom_line(
                aes(y = high.bl, color = id.bl, group = (id.bl)),
                size = 1,
                alpha = 0.75,
                show.legend = FALSE
            ) +
            facet_grid(
                paste(comment, rep, sep = '/') ~ paste(oligo, ramp, sep = '/'),
                scales = 'free_y'
            ) +
            theme_pander() +
            scale_color_viridis_c(
                name = bquote(Delta * epsilon * ' (' * M^-1 ~ cm^-1 * ')')
            )
    ...
    output$p.bases.ui <- renderUI({
        plotOutput("p.bases", width = '100%', height = 600)
    })
    ...
    tm.median <- reactive({
        ...
        for (i in unique(melt.filtered.prep()$group.id)) {
            ...
            gen <- tm.extractoR(
                ...
                mutate(
                    oligo = unique(melt.filtered.group$oligo),
    ...
    output$tm.median <- renderDT({
        tm.median() %>%
            ...
            formatRound(c('tm'), digits = 2)
    })

    output$p.tm.median <- renderPlot({
        tm.median() %>%
            ggplot(aes(x = rep, y = tm.c, fill = ramp)) +
            geom_violin(
                alpha = 0.7,
                trim = TRUE,
                size = 0.75,
            ...
            #              outlier.shape = NA) +
            facet_grid(comment ~ oligo, scales = 'free_y') +
            theme_pander() +
            scale_fill_d3() +
            labs(x = '', y = bquote(T[m] ~ (K)))
    })

    output$p.theta <- renderPlot({
        tm.median() %>%
            ggplot(aes(x = rep, y = theta.t, fill = ramp)) +
            geom_violin(
                alpha = 0.7,
                trim = TRUE,
                size = 0.75,
    ...
    fit.melt.result.summary <- reactive({
        ...
        fit.melt.result.df() %>%
            ...
            mutate(mean.Tm.C = mean(fit.Tm.C), sd.Tm.C = SD(fit.Tm.C))
        # }
    })
    ...
    output$fit.melt.result.summary <- DT::renderDT(server = FALSE, {
        if (file.toggle() == 'no') {
            ...
            datatable(
                ...
                formatRound(
                    c("Tm: derivative (K)", "Tm: derivative (°C)"),
                    digits = 1
                )
        }
    })
    ...
    fit.melt.result.plot <- reactive({
        if (file.toggle() == 'no') {
            ...
            p47 <- fit.melt.result.summary() %>%
                ...
                ggplot() +
                geom_boxplot(
                    aes(x = paste(oligo, comment, sep = "-"), y = tm),
                    color = "grey75"
                ) +
                geom_point(
                    aes(
                        x = paste(oligo, comment, sep = "-"),
                        y = tm,
                        color = ramp,
                        shape = factor(rep)
                    ),
    ...
    switch.format <- reactive({
        if (isTRUE(input$switch.format)) {
            'png'
        } else {
            'pdf'
        }
    ...
    output$dwn.melt.fit <- downloadHandler(
        filename = function() {
            paste("fit", paste0('.', switch.format()), sep = '')
        },
        content = function(file) {
            ggsave(
                ...
                width = as.numeric(input$plot.width),
                height = as.numeric(input$plot.height),
                units = c('cm'),
                dpi = 600
            )
        }
    ...
    palette.modifier <- function(plot = NULL) {
        if (input$select.import.palette.fam == 'd3') {
            ...
            if (input$select.import.palette.fam == "Brewer - qualitative") {
                plot <- plot +
                    scale_color_brewer(palette = input$select.import.palette)
            } else {
                if (input$select.import.palette.fam == "Brewer - sequential") {
                    plot <- plot +
                        scale_color_brewer(
                            palette = input$select.import.palette
                        )
                } else {
                    if (
                        input$select.import.palette.fam == "Brewer - diverging"
                    ) {
                        plot <- plot +
                            scale_color_brewer(
                                palette = input$select.import.palette
                        ...
                        if (input$select.import.palette.fam == "NPG") {
                            ...
                            if (input$select.import.palette.fam == "AAAS") {
                                ...
                                if (input$select.import.palette.fam == "NEJM") {
                                    ...
                                    if (
                                        input$select.import.palette.fam ==
                                            "Lancet"
                                    ) {
        ...
        if (input$select.import.palette.fam.db == 'd3') {
            ...
            if (input$select.import.palette.fam.db == "Brewer - qualitative") {
                ...
                if (
                    ...
                    if (
                        input$select.import.palette.fam.db ==
                            "Brewer - diverging"
                    ) {
                        plot <- plot +
                            scale_color_brewer(
                        ...
                        if (input$select.import.palette.fam.db == "NPG") {
                            ...
                            if (input$select.import.palette.fam.db == "AAAS") {
                                ...
                                if (
                                    input$select.import.palette.fam.db == "NEJM"
                                ) {
                                    plot <- plot +
                                        scale_color_nejm() +
                                        scale_fill_nejm()
        ...
        if (input$select.import.palette.fam == 'd3') {
            ...
            if (input$select.import.palette.fam == 'Brewer - qualitative') {
                ...
                if (input$select.import.palette.fam == 'Brewer - diverging') {
                    ...
                    if (
                        input$select.import.palette.fam == 'Brewer - sequential'
                    ) {
                        pickerInput(
                            "select.import.palette",
                            label = "Choose palette",
    ...
    output$select.import.palette.db <- renderUI({
        if (input$select.import.palette.fam.db == 'd3') {
            ...
            if (input$select.import.palette.fam.db == 'Brewer - qualitative') {
                ...
                if (
                    input$select.import.palette.fam.db == 'Brewer - diverging'
                ) {
                    pickerInput(
                        "select.import.palette.db",
                        label = "Choose palette",
                    ...
                    if (
                        ...
                        if (input$select.import.palette.fam.db == 'IGV') {
                            ...
                            if (
                                input$select.import.palette.fam.db == 'UChicago'
                            ) {
                                pickerInput(
                                    "select.import.palette.db",
                                    label = "Choose palette",
    ...
    melt.palette.modifier <- function(plot = NULL) {
        if (input$select.melting.palette.fam == 'd3') {
            plot <- plot +
                scale_color_d3(
                    palette = input$select.melting.palette,
            ...
            if (input$select.melting.palette.fam == "Brewer - qualitative") {
                ...
                if (input$select.melting.palette.fam == "Brewer - sequential") {
                    ...
                    if (
                        input$select.melting.palette.fam == "Brewer - diverging"
                    ) {
                        plot <- plot +
                            scale_color_brewer(
                                palette = input$select.melting.palette,
                        ...
                        if (input$select.melting.palette.fam == "NPG") {
                            ...
                            if (input$select.melting.palette.fam == "AAAS") {
                                ...
                                if (
                                    input$select.melting.palette.fam == "NEJM"
                                ) {
                                    plot <- plot +
                                        scale_color_nejm(
                                            labels = tm.init.change()$legend
                                    ...
                                    if (
                                        input$select.melting.palette.fam ==
                                            "Lancet"
                                    ) {
                                        plot <- plot +
                                            scale_color_lancet(
                ...
                if (input$select.melting.palette.fam == 'Brewer - diverging') {
                    ...
                    if (
                        ...
                        if (input$select.melting.palette.fam == 'IGV') {
                            ...
                            if (
                                input$select.melting.palette.fam == 'UChicago'
                            ) {
                                pickerInput(
                                    "select.melting.palette",
                                    label = "Choose palette",
